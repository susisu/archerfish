    ...
    it("should run tasks associated to the profile", async () => {
      await tmp.withDir(async dir => {
        ...
        const taskContent = `
          ...
          module.exports = async ({ browser, screenshot, name }) => {
        ...
        const hookContent = `
          ...
          module.exports = async ({ browser, register }) => {
        ...
        const screenshotFilePaths = taskFileNames.map(fileName =>
          path.resolve(screenshotsDirPath, fileName.replace(/\.js$/, ".png"))
        );
        ...
        expect(browser.name).toBe("nyancat");
      }, { unsafeCleanup: true });
    });
  });
});
