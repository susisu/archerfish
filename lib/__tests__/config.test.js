...
const { Config, Profile, configFileName, initConfig } = require("../config");
  ...
  describe("initConfig()", () => {
    it("should initialize a configuration file at the current working directory", async () => {
      await tmp.withDir(async dir => {
        ...
        const content = await readFileAsync(path.resolve(dir.path, configFileName), "utf-8");
    ...
    it("should do nothing if there is already a configuration file", async () => {
      await tmp.withDir(async dir => {
        ...
        await writeFileAsync(path.resolve(dir.path, configFileName), dummyContent, "utf-8");
        await initConfig(dir.path);
        const content = await readFileAsync(path.resolve(dir.path, configFileName), "utf-8");
