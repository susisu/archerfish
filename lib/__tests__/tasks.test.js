...
describe("tasks", () => {
  ...
  describe("runTasks()", () => {
    ...
    it("should be able to run tasks concurrently", async () => {
      await tmp.withDir(async dir => {
        const profile = new Profile("foo", dir.path, {});
        // prepare tasks directory
        const tasksDirPath = profile.tasksDirPath();
    ...
    it("should pass tasks correct arguments", async () => {
      await tmp.withDir(async dir => {
        ...
        expect(browser.args.name).toBe("nyancat");
      }, { unsafeCleanup: true });
    });
  ...
  describe("runHook()", () => {
    it("should run the hook script specified by a key", async () => {
      await tmp.withDir(async dir => {
        ...
        // check
        const browser = await puppeteer.launch();
        const res = await runHook(profile, browser, hookKeys.beforeAll, false);
    ...
    it("should pass tasks correct arguments", async () => {
      await tmp.withDir(async dir => {
        ...
        expect(browser.args.getLogger).toBeInstanceOf(Function);
        expect(browser.args.sleep).toBeInstanceOf(Function);
        expect(browser.args.register).toBe(undefined);
      }, { unsafeCleanup: true });
    });

    it("should be able to register additional arguments", async () => {
      await tmp.withDir(async dir => {
        const profile = new Profile("foo", dir.path, {
          hooks: {
            beforeAll: "hook.js",
        ...
        // check
        const browser = await puppeteer.launch();
        const res = await runHook(profile, browser, hookKeys.beforeAll, true);
        expect(res).toEqual({
          foo : 42,
          bar : null,
          name: "nyancat",
    ...
    it("should throw Error if an unexpected value is tried to be registered", async () => {
      await tmp.withDir(async dir => {
        const profile = new Profile("foo", dir.path, {
          hooks: {
            beforeAll: "hook.js",
        ...
        const browser = await puppeteer.launch();
        await expect(
          runHook(profile, browser, hookKeys.beforeAll, true)
        ).rejects.toThrow(Error, /expects an object or a key-value pair/);
      }, { unsafeCleanup: true });
    });
    ...
    it("should fail if the hook script has failed", async () => {
      await tmp.withDir(async dir => {
        ...
        const browser = await puppeteer.launch();
        await expect(
          runHook(profile, browser, hookKeys.beforeAll, false)
        ).rejects.toThrowError(/hook/);
      }, { unsafeCleanup: true });
    });
    ...
    it("should skip if hook script for the specified key is not defined", async () => {
      await tmp.withDir(async dir => {
        ...
        // check
        const browser = await puppeteer.launch();
        const res = await runHook(profile, browser, hookKeys.afterAll, false);
        expect(res).toBe(undefined);
        expect(browser.hooks).toEqual([]);
      }, { unsafeCleanup: true });
    });

    it("should throw Error if unknown key is specified", async () => {
      const profile = new Profile("foo", "/path/to/project", {});
      const browser = await puppeteer.launch();
      await expect(
        runHook(profile, browser, "unknownHook", false)
      ).rejects.toThrow(Error, /unknown hook/);
    });
  });
});
