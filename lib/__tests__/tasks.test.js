  ...
  describe("runHook()", () => {
    it("should run the hook script specified by a key", async () => {
      await tmp.withDir(async dir => {
        ...
        // check
        const browser = await puppeteer.launch();
        await runHook(profile, browser, hookKeys.beforeAll);
    ...
    it("should fail if the hook script has failed", async () => {
      await tmp.withDir(async dir => {
        ...
        // check
        const browser = await puppeteer.launch();
        await expect(runHook(profile, browser, hookKeys.beforeAll)).rejects.toThrowError(/hook/);
      }, { unsafeCleanup: true });
    });
    ...
    it("should skip if hook script for the specified key is not defined", async () => {
      await tmp.withDir(async dir => {
        ...
        // check
        const browser = await puppeteer.launch();
        await runHook(profile, browser, hookKeys.afterAll);
    ...
    it("should throw Error if unknown key is specified", async () => {
      ...
      const browser = await puppeteer.launch();
      await expect(runHook(profile, browser, "unknownHook")).rejects.toThrow(Error, /unknown hook/);
    });
  });
});
